...
class CanvasRepositoryImpl @Inject constructor(
    ...
    override suspend fun generateScribbleTitle(): Int? = withContext(Dispatchers.IO) {
        scribbleDao.getMaxScribbleNumber()
    }

    override suspend fun getCanvasById(canvasId: Long): CanvasDrawing? = withContext(Dispatchers.IO) {
        scribbleDao.getCanvasById(canvasId = canvasId)?.toCanvasDrawing()
    }

    override suspend fun getCanvasesByIds(canvasIds: Set<Long>): List<CanvasDrawing> = withContext(Dispatchers.IO) {
        scribbleDao.getCanvasesByIds(ids = canvasIds).map { it.toCanvasDrawing() }
    }

    override suspend fun upsertCanvas(canvasDrawing: CanvasDrawing): Long = withContext(Dispatchers.IO) {
        scribbleDao.upsertCanvas(canvasEntity = canvasDrawing.toCanvasEntity())
    }

    override suspend fun deleteCanvases(canvases: List<CanvasDrawing>): Int = withContext(Dispatchers.IO) {
        scribbleDao.deleteCanvases(canvases = canvases.map { it.toCanvasEntity() })
    }

    override suspend fun deleteCanvasesById(canvasIds: Set<Long>): Int = withContext(Dispatchers.IO) {
        scribbleDao.deleteCanvasesById(canvasIds = canvasIds)
    }

    override suspend fun deleteOldRecycledCanvases(timeStampLimit: Long): Int = withContext(Dispatchers.IO) {
        scribbleDao.deleteOldRecycledCanvases(timeStampLimit)
    }

    override suspend fun recycleCanvases(canvasIds: Set<Long>, timeStamp: Long): Int = withContext(Dispatchers.IO) {
